	...
	public static function filter_rows( $table, $render_options ) {
		...
		foreach ( $filter_data as $row_idx => $row ) {
			...
			switch ( $compare ) {
				case 'none':
				case 'or':
					if ( ! in_array( true, $found, true ) ) // at least one word was found / only filter word was found
						$hidden_rows[] = $row_idx;
					break;
				case 'and':
					if ( in_array( false, $found, true ) ) // if not (at least one word was *not* found) == all words were found
						$hidden_rows[] = $row_idx;
					break;
			}
		}
